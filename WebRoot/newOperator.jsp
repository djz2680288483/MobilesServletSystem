...
<script>
	$(document).ready(function(){
		...
		T1.blur(function(){
			...
			var value = T1.val();
			//使用ajax异步验证
			$.get("check1",{operator_id:value,operator:'newOperator'},function(data){
				
				if(data=="ok"){
					flag=true;
					sp1.html("<font color='green'>ID可用</font>"); 
				}else if(data=="not ok"){
					flag=false;
					sp1.html("<font color='red'>ID不可用</font>"); 
				}else{
					flag=false;
					sp1.html(""); 
				}
...
<table cellSpacing=0 cellPadding=0 width=749 border=0>
  ...
  <tr vAlign=top>
    ...
    <form method="POST" action="operator" id="form1">
      <input type="hidden" name="operator" value="addOperator"/>
      <p>登录ID：<input type="text" name="T1" size="20" value="" id="T1"><span id="sp1"></span></p>
      ...
      <p>是否管理员：<input type="radio" value="1" checked name="R1">是&nbsp;&nbsp;&nbsp; 
      <input type="radio" name="R1" value="0">否</p>
      <p><input type="button" value="提交" name="B1" id="btn_sub"></p>
    </form>
    </td>
    </tr>
  </tbody></table>
